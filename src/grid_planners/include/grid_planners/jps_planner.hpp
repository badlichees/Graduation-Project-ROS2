...
private:
  // 跳跃方向本身携带剪枝上下文
  int jump(int x, int y, int dx, int dy, int gx, int gy, nav2_costmap_2d::Costmap2D * cm) const;

  bool blocked(int x, int y, nav2_costmap_2d::Costmap2D * cm) const;

  // 跳点路径需要补成 Nav2 控制器更容易消费的连续路径
  nav_msgs::msg::Path buildJPSPath(const std::vector<int> & parent, int goal_idx,
