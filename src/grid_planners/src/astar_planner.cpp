  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  g_cost[s_idx] = 0.0f;
  open.push(
    {this->priority(0.0f, this->heuristic(s_idx, static_cast<int>(gx), static_cast<int>(gy), W)),
      s_idx});
  ...
  while (!open.empty()) {
    ...
    for (int d = 0; d < 8; ++d) {
      ...
      const float terrain =
        (cost == nav2_costmap_2d::NO_INFORMATION) ? 0.5f : 0.01f * static_cast<float>(cost);
      ...
      if (ng < g_cost[nidx]) {
        ...
        parent[nidx] = cur;
        open.push(
          {this->priority(ng, this->heuristic(nidx, static_cast<int>(gx), static_cast<int>(gy), W)),
            nidx});
      }
    }
  }
...
nav_msgs::msg::Path AStarPlanner::buildPath(const std::vector<int> & parent, int goal_idx,
  nav2_costmap_2d::Costmap2D * costmap, const std::string & frame_id, const rclcpp::Time & stamp)
{
