...
void RRTStarPlanner::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent,
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(node, name_ + ".max_iterations",
    rclcpp::ParameterValue(5000));
  nav2_util::declare_parameter_if_not_declared(node, name_ + ".step_size",
    rclcpp::ParameterValue(0.5));
  nav2_util::declare_parameter_if_not_declared(node, name_ + ".search_radius",
    rclcpp::ParameterValue(1.5));
  nav2_util::declare_parameter_if_not_declared(node, name_ + ".goal_tolerance",
    rclcpp::ParameterValue(0.5));
  nav2_util::declare_parameter_if_not_declared(node, name_ + ".goal_bias",
...
void RRTStarPlanner::updateRuntimeParameters()
{
  auto node = node_.lock();
  if (!node) {
    return;
  ...
  max_iterations_ =
    std::max(1, static_cast<int>(node->get_parameter(name_ + ".max_iterations").as_int()));
  step_size_ = std::max(0.01, node->get_parameter(name_ + ".step_size").as_double());
  search_radius_ = std::max(0.05, node->get_parameter(name_ + ".search_radius").as_double());
  goal_tolerance_ = std::max(0.01, node->get_parameter(name_ + ".goal_tolerance").as_double());
  goal_bias_ = std::clamp(node->get_parameter(name_ + ".goal_bias").as_double(), 0.0, 1.0);
  allow_unknown_ = node->get_parameter(name_ + ".allow_unknown").as_bool();
}

nav_msgs::msg::Path RRTStarPlanner::createPlan(const geometry_msgs::msg::PoseStamped & start,
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  RCLCPP_INFO(logger_, "RRT* createPlan: (%.2f,%.2f) -> (%.2f,%.2f)", start.pose.position.x,
  ...
  for (int iter = 0; iter < max_iterations_; ++iter) {
    ...
    if (new_x < ox || new_x >= max_x || new_y < oy || new_y >= max_y) {
      continue;
    }

    unsigned int nmx, nmy;
    if (!cm->worldToMap(new_x, new_y, nmx, nmy)) {
      continue;
    }
    ...
    for (int nid : near_ids) {
      const double new_cost = best_cost + std::hypot(new_x - nodes[nid].x, new_y - nodes[nid].y);
      if (new_cost < nodes[nid].cost && isCollisionFree(new_x, new_y, nodes[nid].x, nodes[nid].y)) {
  ...
  if (goal_node_idx == -1) {
    const double ms =
      std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - t0)
        .count();
    publishPlannerStats(stats_pub_, name_, ms, 0.0, static_cast<int>(nodes.size()), false);
  ...
  RCLCPP_INFO(logger_, "RRT* found path: %zu waypoints -> %zu interpolated poses", waypoints.size(),
    path.poses.size());
  return path;
}
...
int RRTStarPlanner::nearestNode(const std::vector<Node> & nodes, double x, double y) const
{
  ...
  for (int i = 0; i < static_cast<int>(nodes.size()); ++i) {
    const double d2 = (nodes[i].x - x) * (nodes[i].x - x) + (nodes[i].y - y) * (nodes[i].y - y);
    if (d2 < best_d2) {
      best_d2 = d2;
      best = i;
    }
...
std::vector<int> RRTStarPlanner::nearNodes(const std::vector<Node> & nodes, double x,
  double y) const
{
